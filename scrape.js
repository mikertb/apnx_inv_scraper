...
function isLogged(){
    if(!fs.existsSync('logs')) fs.mkdirSync('logs');
    if(!fs.existsSync(config.auth_last_file)) fs.writeFileSync(config.auth_last_file,'0');
    let last_log = Number(fs.readFileSync(config.auth_last_file).toString());
...
function login(callback){
    ...
    function writeCookie(data){
        ...
        fs.writeFileSync(config.cookie_file,cookies_array.join('; '));
    }
    request.post(data,function(error,response,body){
        ...
        if(!error){
            if(response.headers.location == 'https://console.appnexus.com/buyside/advertiser') {
                ...
                fs.writeFileSync(config.auth_last_file,date);
                logger('login_access','Ok: Login successful.');
                success = true;
            }
            else {
                logger('login_errors','Error: Login failed.');
            }
        ...
        else{logger('login_errors',error.message);}
        if(success) {
...
function keepAlive(callback) {
    if(!isLogged()){
        if(login_attempts < 3) {
            login_attempts++;
            logger('login_access','NOTICE: Previous session has expired.');
            console.log('NOTICE: Previous session has expired.');
            logger('login_access','Login attempt '+login_attempts);
            login(callback);
        } else {
            logger('login_errors','ERROR: Login failed permanently with attempts '+login_attempts);
        }
...
function getCSV(country,device,supply,filename){
    ...
    let data = {
        ...
        headers: { Cookie: fs.readFileSync(config.cookie_file).toString() }
    }
    request(data,function(error,response,body){
        if(!error){
            ...
            if(!checkcsv(body)) {
                logger('csv_errors','Can\'t read csv for "'+country.name+'"');
            } else {
                updateProgress();
                fs.writeFileSync('logs/login_body.log',body);
            }
        }else{ logger('csv_errors','ERROR: "'+error.message+'" for geo '+country.name+'('+country.code+')'); }
    });
...
country_delay = 0;
keepAlive(()=>{
    logger('app_run','Scraping started.');
    updateProgress(true);
